...
export class ContentController {
  ...
  async getMovies(
    ...
    @Query('sortBy', new DefaultValuePipe('updatedAt')) sortBy: string = 'updatedAt',
  ...
  @UseGuards(RbacGuard)
  @RequirePermissions(PERMISSIONS.CONTENT_MOVIES_CREATE)
  @ApiOperation({ summary: 'Bulk import movies' })
  async bulkImportMovies(
    @Body() { movies }: { movies: Array<any> },
    @CurrentUser('userId') userId: string,
  ) {
    const result = await this.movieService.bulkImport(movies);
  ...
  async createSeason(
    ...
    const result = await this.seriesService.createSeason(seriesId, createSeasonDto);
  ...
  async createEpisode(
    ...
    const result = await this.seriesService.createEpisode(seasonId, createEpisodeDto);
  ...
  async getPopularGenres(@Query('limit', new DefaultValuePipe(10), ParseIntPipe) limit: number = 10) {
  ...
  async getPopularTags(@Query('limit', new DefaultValuePipe(20), ParseIntPipe) limit: number = 20) {
  ...
  async addToCollection(
    ...
    const result = await this.collectionService.addItem(collectionId, addItemDto);
  ...
  async reorderCollection(
    @Param('id') collectionId: string,
    @Body() { itemOrders }: { itemOrders: Array<{ id: string; order: number }> },
    @CurrentUser('userId') userId: string,
  ) {
    const result = await this.collectionService.reorderItems(collectionId, itemOrders);
  ...
  async reorderHomepageSections(
    @Body() { sectionOrders }: { sectionOrders: Array<{ id: string; order: number }> },
    @CurrentUser('userId') userId: string,
  ) {
