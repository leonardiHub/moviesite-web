...
export class MeilisearchService implements OnModuleInit {
  ...
  async indexMovie(movie: any) {
    ...
    const document = {
      ...
      backdrop_url: movie.artworks?.find((a: any) => a.kind === "backdrop")
        ?.url,
  ...
  async searchMovies(
    ...
    const searchParams: any = {
      ...
      attributesToRetrieve: options?.attributes || [
        "id",
        "title",
        "synopsis",
        "year",
  ...
  async indexSeries(series: any) {
    ...
    const document = {
      ...
      creators:
        series.credits
          ?.filter((c: any) => c.role === "creator")
          .map((c: any) => c.person.name) || [],
      poster_url: series.artworks?.find((a: any) => a.kind === "poster")?.url,
      backdrop_url: series.artworks?.find((a: any) => a.kind === "backdrop")
        ?.url,
  ...
  async globalSearch(query: string, options?: any) {
    ...
    return {
      ...
      people: people.hits,
      total:
        (movies.estimatedTotalHits || 0) +
        (series.estimatedTotalHits || 0) +
        (people.estimatedTotalHits || 0),
    };
  }
